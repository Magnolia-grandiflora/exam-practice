...
- Markdown 导出、Windows PDF 打印与答题卡图片阅卷；识别结果须人工确认。A3 试卷采用横向双栏，答题卡保持 A4 纵向。
...
- `SHA256SUMS-0.1.0.txt`：以上两个安装包的 SHA-256 校验和。

源码附 MIT License、第三方声明、15 题测试包及同步后端部署材料。Windows 包含阅卷组件和依赖许可文本。

## 安装与验证边界

Windows 完整发布包与 Android release APK 已生成，版本为 0.1.0+1、名称为“题序 / Exam Practice”。本次发布前重新运行静态分析与全量测试，并检查两个安装包内容；Windows 包含阅卷组件及依赖许可文本。两个成品的 SHA-256 列于随 Release 提供的 `SHA256SUMS-0.1.0.txt`。

安装包仅附 15 道功能演示题，没有正式考试题库、个人作答数据库或预置个人云账号。云同步需自行配置。
