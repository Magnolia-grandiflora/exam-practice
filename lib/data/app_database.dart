...
class AppDatabase {
  ...
  void _initialize() {
    ...
    _migrateV5PaperHistoryVisibility();
  }

  void _migrateV5PaperHistoryVisibility() {
    if (_db.select('SELECT 1 FROM schema_migrations WHERE version=5').isNotEmpty) {
      return;
    }
    _db.execute('BEGIN');
    try {
      _db.execute(
        'ALTER TABLE paper_attempts ADD COLUMN hidden_locally INTEGER NOT NULL DEFAULT 0',
      );
      // 旧版本同步归档不修改 updated_at_utc；手动删除会修改它。
      // 保留本机删除及已修改的远端删除，恢复自动归档的远端试卷。
      _db.execute('''
        UPDATE paper_attempts SET hidden_locally=1
        WHERE archived_locally=1 AND (
          source_device_id=? OR updated_at_utc<>created_at_utc
        )
      ''', [deviceId]);
      _db.execute(
        'INSERT INTO schema_migrations(version, applied_at_utc) VALUES(5, ?)',
        [_now()],
      );
      _db.execute('COMMIT');
    } catch (_) {
  ...
  List<AttemptSummary> history({int limit = 100}) => _db
    ...
    WHERE a.hidden_locally=0 AND (a.archived_locally=0 OR ?=0)
    GROUP BY a.attempt_id ORDER BY a.started_at_utc DESC LIMIT ?
  ''',
        [archivesRemotePapers ? 1 : 0, limit],
      )
      .map(
        (row) => AttemptSummary(
  ...
  void archiveAttempt(String attemptId) {
    _db.execute(
      'UPDATE paper_attempts SET hidden_locally=1,updated_at_utc=? WHERE attempt_id=?',
      [_now(), attemptId],
    );
    ...
    if (existing.isNotEmpty) {
      ...
      ''',
        [
          ...
          archivesRemotePapers && !ownDevice ? 1 : 0,
          now,
          now,
        ],
      );
